...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,img_size=[224,224],
        ...
        pad_b_img = (self.n_win - w % self.n_win) % self.n_win
        pad_r_img = (self.n_win - h % self.n_win) % self.n_win

        h, w = h + pad_r_img, w +pad_b_img
        self.img_h=h
        self.img_w=w
        self.img_min=min(h,w)
        #assert h==w
        # 窗口大小
        self.ws_h=h//self.n_win
        self.ws_w = w// self.n_win
        self.ws_min=min(self.ws_h,self.ws_w)

        # 得到偏移参数
        self.sampling_offsets = nn.Sequential(
            nn.AvgPool2d(kernel_size=self.ws_min, stride=self.ws_min),
            nn.LeakyReLU(),
        ...
        window_reference = nn.functional.avg_pool2d(image_reference, kernel_size=(self.ws_h,self.ws_w))
        ...
        base_coords_h = torch.arange(self.ws_h) * 2 * self.ws_h / self.ws_h / (h - 1)
        base_coords_h = (base_coords_h - base_coords_h.mean())
        base_coords_w = torch.arange(self.ws_w) * 2 * self.ws_w / self.ws_w / (w - 1)
        ...
        assert expanded_base_coords_h.shape[1] == self.ws_h
        ...
        assert expanded_base_coords_w.shape[1] == self.ws_w
        ...
        # coords [1,2,n_win,ws_h,n_win,ws_w]
        # coords [1,2,window_num_h,ws_h,window_num_w,ws_w]
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        sampling_offsets_x = self.sampling_offsets(x)
        #sampling_offsets=sampling_offsets_x[:,:,0:window_num_h,0:window_num_w]
        sampling_offsets = F.interpolate(sampling_offsets_x, size=(self.n_win, self.n_win), mode='bilinear', align_corners=False)
        #num_predict_total = N
        ...
        sampling_scales_x = self.sampling_scales(x)  # N, 2, window_num_h, window_num_w
        sampling_scales=F.interpolate(sampling_scales_x, size=(self.n_win, self.n_win), mode='bilinear', align_corners=False)
        ...
        if(H==self.img_h):
          # 从第三个维度中随机选择228个索引
          selected_indices = np.random.choice(self.ws_w, W // self.n_win, replace=False)
          # 根据选定的索引切片第三个维度
          sample_coords_big = coords[:,:,:,:,:,selected_indices]
        else:
          # 从第二个维度中随机选择228个索引
          selected_indices = np.random.choice(self.ws_w, H // self.n_win, replace=False)
          coords=coords.permute(0,1,4,5,2,3)
          # 根据选定的索引切片第二个维度
          sample_coords_big = coords[:,:,:,selected_indices,:,:]

        sample_coords = sample_coords_big.permute(0, 2, 3, 4, 5, 1).reshape(N, H,
                                                                 W, 2)
