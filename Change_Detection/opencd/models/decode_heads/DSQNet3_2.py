...
class DSQNet3_2(BaseDecodeHead):
    ...
    def __init__(self, img_size=512,interpolate_mode='bilinear', **kwargs):
        ...
        self.regions_cor = ConvModule(
            in_channels=self.channels//2,
            out_channels=self.channels//2,
            kernel_size=1,
            norm_cfg=self.norm_cfg)
        self.regions_cor2 = ConvModule(
            in_channels=self.channels//2,
            out_channels=self.channels//2,
            kernel_size=1)
    ...
    def BMQ(self,x1,x2,reference_area,query_key):
        ...
        out1_content=out1_content-torch.einsum('ijkl,ijlm->ijkm',F.softmax(regions_relevance,dim=3),out2_content)
        out2_content=out2_content-torch.einsum('ijkl,ijlm->ijkm',F.softmax(regions_relevance.permute(0,1,3,2),dim=3),out1_content)
