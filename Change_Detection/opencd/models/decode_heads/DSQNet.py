...
class DSQNet(BaseDecodeHead):
    ...
    def BMQ(self,x1,x2,reference_area,query_key):
        ...
        points1 = points1.view(b * self.window_nums, 1,1,2).tanh()

        out1_content = F.grid_sample(out1_content, points1, padding_mode="zeros", align_corners=False).view(b * self.window_nums,-1)
        out1_content = out1_content.view(b, self.window_nums, -1).permute(0, 2, 1)# (b,128,c) ->(b,c,128)
        #print(out1_content.shape)
        out1_content=out1_content.view(b, c, int(self.window_nums//8),8)  # (b,c, self.window_nums//2,self.window_nums//2)
        ...
        points2 = points2.view(b * self.window_nums, 1, 1, 2).tanh()

        out2_content = F.grid_sample(out2_content, points2, padding_mode="zeros", align_corners=False).view(
            b * self.window_nums, -1)
        out2_content = out2_content.view(b, self.window_nums, -1).permute(0, 2, 1)  # (b,32,c) ->(b,c,32)
        # print(out1_content.shape)
        out2_content = out2_content.view(b, c, int(self.window_nums//8),
                                         8)  # (b,c, self.window_nums**0.5,self.window_nums**0.5)
