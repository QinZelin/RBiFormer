...
class CFP(BaseModule):
    def __init__(self,
        ...
        self.query_bias_conv = nn.Conv2d(self.mlp_channels, self.mlp_channels, kernel_size=1, stride=1, padding=0)
...
class Changer_CAI2_3_one(BaseDecodeHead):
    """The Head of Changer.
    ...
    def __init__(self, interpolate_mode='bilinear', **kwargs):
        ...
        self.change_areas =128
        self.reference_area = nn.Embedding(self.change_areas, 4)
        ...
        self.sub_norm_conv = ConvModule(
            in_channels=self.change_areas,
            out_channels=self.change_areas,
            kernel_size=1,
            norm_cfg=self.norm_cfg)
    ...
    def CAI(self,x1,x2,reference_area,query_bias1,query_bias2):
        b, c, h, w = x1.shape
        reference_area1=reference_area.unsqueeze(0).repeat(b, 1, 1)
        ...
        reference_area2 = reference_area.unsqueeze(0).repeat(b, 1, 1)
    ...
    def forward(self, inputs):
        ...
        b, c, h, w = out1.shape
        query_bias1_1=self.sub_norm_conv(query_bias1-query_bias2)
        query_bias2_2=self.sub_norm_conv(query_bias2-query_bias1)
        out1,out2=self.CAI(out1,out2,self.reference_area.weight,query_bias1_1,query_bias2_2)
