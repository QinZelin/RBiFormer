    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            ...
            padded_img_ori = mmcv.impad(
                results['img_original'], shape=self.size, pad_val=self.pad_val)

        elif self.size_divisor is not None:
            padded_img = mmcv.impad_to_multiple(
                results['img'], self.size_divisor, pad_val=self.pad_val)
            padded_img_ori = mmcv.impad_to_multiple(
                results['img_original'], self.size_divisor, pad_val=self.pad_val)
        ...
        results['img_original']=padded_img_ori
    def _pad_seg(self, results):
        """Pad masks according to ``results['pad_shape']``."""
        for key in results.get('seg_fields', []):
            results[key] = mmcv.impad(
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        results['img_original']=results['img']
        results['img'] = mmcv.imnormalize(results['img'], self.mean, self.std,
                                          self.to_rgb)
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
